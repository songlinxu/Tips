...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        input_prompt_1 = self.dataframe.iloc[idx]['past_question_answer_prompt']
        input_prompt_2 = self.dataframe.iloc[idx]['future_question_prompt']
        label = self.dataframe.iloc[idx]['correctness']
...
class Experiment_Pipeline():
    ...
    def model_init(self,load_model_type,lr=1e-4):
        ...
        # self.model = Bert_KT('bert-base-uncased').to(self.device)
...
def run_exp_bertKT():
    dataset_path = '/home/songlin/dataset/GKT/eduagent/dataset_gkt_org.csv'
    log_folder = '/home/songlin/study_results/GKT/bertKT/'
    
    experiment_pipeline = Experiment_Pipeline(512,log_folder,load_model_type='none',lr=1e-5)
    experiment_pipeline.dataset_prepare(dataset_path)
    experiment_pipeline.model_train(epochs=8,vis=True)
